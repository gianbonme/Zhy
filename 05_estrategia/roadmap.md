# 🎯 Roadmap de Execução (Masterplan Agressivo)

> **Documento Vivo: O Mapa de Batalha Zhy**  
> Estratégia de dominação rápida de mercado dividida em **5 Sprints Incisivos**. O objetivo é tracionar receita massiva nos primeiros 6 meses via produtos MAPA (rápido fluxo de caixa), pavimentando a sustentação e o lucro bruto para a linha high-ticket ANVISA.

---

## 🟡 FASE 1: O "Go-To-Market" Inicial [Mês 01 - 02]

**Foco**: Penetração imediata de rotina, geração de barulho no mercado e MVP de logística.
**Regulação**: MAPA (Aprovação ágil)

| Produto | Parceiro / Produção | Status Atual |
| :--- | :--- | :--- |
| **🧃 Sucos Cold Press (6 sabores)** | Vida Líquida | 9 Fórmulas OK. Foco D2C/B2B |
| **☕ Cápsulas Nespresso (Cx 10)** | MushSpresso | Lote 9000 cápsulas definido. Embalagem interim (Caco/Carol). |

🔥 **Prioridades de Ataque (Imediatas)**:
- Fechar Parceiro HPP no Rio de Janeiro (Gargalo Mestre).
- Orçar e rodar embalagens PETs definitivas para os sucos.
- Fechar comercial de caixas (custo/benefício B2C) focando em bater o preço do protótipo.
- Iniciar fechamento intenso de pontos Varejo Premium.

---

## 🟠 FASE 2: Expansão de Rotina (Cross-Sell) [Mês 03 - 04]

**Foco**: Aproveitar o trilho logístico e o insumo da Fase 1, aumentando brutalmente o LTV do cliente.
**Regulação**: MAPA

| Produto | Parceiro / Produção | Status Atual |
| :--- | :--- | :--- |
| **🧴 Shots Funcionais (Imunidade/Foco)** | Vida Líquida | Receitas conceituadas |
| **🫘 Café Moído Premium** | MushSpresso | Rascunho inicial do pacote |

🔥 **Prioridades de Ataque**:
- Definir as mini-garrafas PET.
- Desenvolver a embalagem "luxo" do café com a Rebu (previsão de 4 meses).
- Criar a mecânica de venda conjunta (Sucos + Shots) para alavancagem de Ticket.

---

## 🟢 FASE 3: A Máquina de Solúveis [Mês 05 - 06]

**Foco**: Margem de operação altíssima (Branding + P&D próprio).
**Regulação**: MAPA

| Produto | Produção | Status Atual |
| :--- | :--- | :--- |
| **🍵 Blends: Cacau, Matcha, Golden Latte** | P&D Zhy / Co-Packer | Testes de receita P&D |

🔥 **Prioridades de Ataque**:
- Encontrar as indústrias certeiras para envase e mistura inox.
- Fechar as formulações (zerar stevia, adotar outro veículo orgânico).
- Definir os potes premium/latas que farão esse produto um item estético "de bancada".

---

## 🔵 FASE 4: Dominação de Escala (RTD) [Mês 07 - 09]

**Foco**: Presença visual ostensiva no varejo físico e margem por volume explosivo.
**Regulação**: MAPA

| Produto | Produção | Status Atual |
| :--- | :--- | :--- |
| **🥫 Bebidas e Sodas em Lata (Ice Coffee, etc)** | Co-Packer Premium | Blueprint Estratégico Futuro |

🔥 **Prioridades de Ataque**:
- Originar contato com mega-indústrias (bebidas prontas) para terceirizar tudo.
- Estruturar a distribuição nacional "Grab & Go".
- Estruturar transição da produção de café para o Rio de Janeiro (Café próprio/Controle total).

---

## 🟣 FASE 5: A Coroa (Suplementação Clínica) [Mês 10 - 15+]

**Foco**: Autoridade absoluta científica, branding supremo e High-Ticket.
**Regulação**: ANVISA (Burocracia brutal, precisa iniciar *ONTE*M no paralelo)

| Produto | Produção | Status Atual |
| :--- | :--- | :--- |
| **💊 Cogumelos Isolados e Zhy Blend Base** | Lab Próprio RJ (Equip. OK) | Dossiês com Adriana (ANVISA) |

🔥 **Prioridades de Ataque**:
- Acelerar aprovações com os dossiês (Advogada).
- Preparar a narrativa de venda superpremium da linha mãe, justificando a pureza frente aos concorrentes "brancos/limpos".

---

## ⚡ TIMELINE TÁTICA E INCISIVA

Chega de laxeza (anos). Trabalhamos com Sprints semestrais, no limite da agressividade logística. O cronograma reverso focado em gerar caixa já é o seguinte:

```text
[MÊS 01 - 02] ──► FASE 1: Sucos & Cápsulas (Ataque Go-To-Market Imediato)
      │
[MÊS 03 - 04] ──► FASE 2: Shots & Café Moído (Expansão e Cross-Sell Rápido)
      │
[MÊS 05 - 06] ──► FASE 3: Solúveis Premium (Margem Líquida Alta - Cacau/Matcha)
      │
[MÊS 07 - 09] ──► FASE 4: Enlatados RTD (Escala e Dominação em Volume)
      │
[MÊS 10 - 15] ──► FASE 5: Suplementos Mãe (Liberações ANVISA / High-Ticket) 
```

> **Atenção (Gargalo ANVISA)**: A regulação da Fase 5 e o trabalho nos bastidores rodam paralelos a todas as outras. A fábrica física RJ e as obrigações judiciais ditam o compasso das cápsulas, enquanto todo o caixa para bancar essa espera sai do varejo de MAPA nas Fases 1 a 4.

---

## 🔮 O IMPÉRIO ZHY — MARCOS DE LONGO PRAZO

As 5 fases acima formam o "Tático de Ataque Imediato" da Zhy. Em termos de horizontes temporais maiores:

### Curto prazo (2026–2027)
- Execução cirúrgica da **Fase 1** (Sucos e Café em Cápsula).
- Estabelecer os canais inaugurais de receita (B2B Varejo Premium, D2C Assinaturas).
- Construir autoridade e conteúdo esteticamente impecável educando a massa leiga sobre Adaptógenos e MTC.

### Médio prazo (2027–2028)
- Implementação avassaladora da **Fase 3** (Solúveis com Produção Própria via P&D laboratorial).
- Desova maciça da **Fase 4** (Latas RTD de alta aceitação em farmácias, mercados).
- Conselho Médico Integrativo e Científico formado para blindar a chancela na Mídia.

### Longevo Absoluto (2028+)
- Liberação burocrática integral pela ANVISA e desova ostensiva da **Fase 5** (Suplementação Clínica Encapsulada Mãe).
- Desenvolvimento/Patente de tecnologias baseadas em **Proteína de Micélio** (Alimentos funcionais alternativos - Oceano inexplorado no Brasil).
- Mapeamento Logístico para a primeira remessa de produtos latinos (LatAm) e Euro (Portugal).
- Oásis Sensorial Flagship: Abertura da primeira megastore ou café conceito Zhy (hub de comunidade).
