# 🚀 PLANO DE LANÇAMENTO ZHY — 30 / 60 / 90 DIAS

> **Data-base:** 25/03/2026 • **Meta D-Day Fase 1:** ~01/05/2026 (~5 semanas)
> **Produtos Fase 1:** Sucos Cold Press + Cápsulas Nespresso
> **Produtos Fase 2 (45-75d):** 4 Shots Cold Press + Café Moído
> **Produtos Fase 3 (90-120d):** Linha Solúveis (Golden Latte, Cacau, Matcha)

---

## 🎯 METAS GERAIS (Norte Magnético)

| # | Meta | Prazo | KPI |
|---|------|-------|-----|
| 1 | Lançar **Sucos Cold Press** e **Cápsulas Nespresso** no mercado | 37 dias | Produto à venda (D2C + B2B) |
| 2 | E-commerce D2C operacional (site c/ checkout) | 30 dias | Site live com carrinho |
| 3 | Ativar **primeiros 10 pontos B2B** premium no RJ | 60 dias | PDVs ativos |
| 4 | Lançar **4 Shots Cold Press** e **Café Moído** | 75 dias | Produto à venda |
| 5 | Atingir **4.000 unidades vendidas** (3k sucos + 1k cx cápsulas) | 120 dias | Unidades vendidas |
| 6 | **20 pontos B2B** ativos | 120 dias | PDVs ativos |
| 7 | Iniciar produção da **Linha Solúveis** (Golden Latte, Cacau, Matcha) | 90 dias | P&D concluído, parceiro industrial definido |

---

## 🔴 GARGALOS CRÍTICOS (Resolver PRIMEIRO)

> [!CAUTION]
> Esses itens travam TODO o resto. Sem resolvê-los, o lançamento não acontece.

| # | Gargalo | Responsável | Deadline | Status |
|---|---------|-------------|----------|--------|
| 1 | **Fechar parceiro HPP no RJ** (sem HPP, suco dura 3 dias) | Guilherme | **28/03** | 🔴 Em Processo |
| 2 | **Fechar embalagem Cápsulas** (caixa final p/ venda) | Marketing (Caco/Carol) | **28/03** | 🟡 Em Processo |
| 3 | **Definir & orçar garrafas PET** (sucos + 4 shots) | Gianluca | **03/04** | 🔴 Investigar |
| 4 | **Maquinário** (Rotuladora do Alvaro + Freezers) | Gianluca | **10/04** | 🔴 Investigar |
| 5 | **Criar rótulos dos sucos** | Marketing (Caco/Carol) | **10/04** | 🔴 Iniciar |
| 6 | **Finalizar site c/ checkout** | Web/Programação | **20/04** | 🟡 Em Andamento |
| 7 | **Constituir Conselho Científico Zhy** (médico funcional + nutricionista sênior + especialista cogumelos) — necessário para validar formulações antes da submissão ao MAPA | Gianluca | **URGENTE** | 🔴 A Iniciar |
| 8 | **Encontrar parceiro industrial para Solúveis** (envase + mixer inox) | Gianluca | **30/04** | 🔴 A Iniciar |

---

## ✅ CHECKLIST SEMANAL (30 DIAS — FASE 1)

### SEMANA 1 (25/03 → 31/03) — "DESTRAVE"

- [ ] **HPP** — Ligar para os 3 contatos de máquinas HPP no RJ (Guilherme). Follow-up diário.
- [ ] **Cápsulas** — Revisar e aprovar arte final da embalagem com Caco/Carol.
- [ ] **Cápsulas** — Cotação alternativa de caixas (bater preço/qualidade vs Adriel).
- [ ] **PET** — Iniciar pesquisa de fornecedores PET para sucos e shots (formato ideal HPP, custo unitário).
- [ ] **Regulação** — Check-in com Priscila (MAPA) sobre status das liberações.
- [ ] **MushSpresso** — Confirmar status do molde de Brasília e lote de 9.000 cápsulas.
- [ ] **Shots** — Formulações finalizadas ✅ (4 shots: BlindC, Inflamação, Foco, Rendimento). Próximo: validar com nutricionista.
- [ ] **Solúveis** — Formulações P&D concluídas ✅ (Golden Latte, Cacau, Matcha). Próximo: buscar parceiro industrial.

### SEMANA 2 (01/04 → 07/04) — "EMBALAGENS E PRODUÇÃO"

- [ ] **HPP** — Status definitivo: parceiro confirmado? Se não, plano B (outro estado ou outra tecnologia).
- [ ] **Hiring** — Iniciar processo de recrutamento para **Vendedor B2B** e **Braço Direito**.
- [ ] **Embalagem sucos** — Fechar formato PET, enviar brief para design de rótulo (Caco/Carol).
- [ ] **Embalagem cápsulas** — Aprovar arte + mandar produzir caixas finais.
- [ ] **Embalagem shots** — Definir garrafa PET 60ml para os 4 shots; orçar fornecedores.
- [ ] **Embalagem café moído** — Definir material do pacote, criar briefing para Rebu.
- [ ] **Serigrafia** — Iniciar testes nos potes de vidro (brandbook).
- [ ] **Produção** — Encomendar matéria-prima complementar para as 9k cápsulas.
- [ ] **Site** — Verificar progresso do checkout / payment gateway.
- [ ] **Shots** — Cotação real de matéria-prima dos 4 shots com fornecedores.

### SEMANA 3 (08/04 → 14/04) — "PRIMEIROS LOTES"

- [ ] **Produção sucos** — Agendar primeiro lote piloto na Vida Líquida (Alvaro).
- [ ] **Produção cápsulas** — Confirmar envio do lote de 900 caixas (MushSpresso).
- [ ] **Rotuladora** — Fechar compra/aluguel da rotuladora do Alvaro.
- [ ] **Freezers** — Comprar freezers para acoplar na fábrica.
- [ ] **Site** — Finalizar estrutura de checkout e testar fluxo de compra completo.
- [ ] **Marketing** — Criar rótulos dos sucos (ARTE FINAL).
- [ ] **Marketing** — Preparar fotos de produto e copy para lançamento.
- [ ] **Shots** — Iniciar testes de palatabilidade das 4 fórmulas (equilibrar sabor vs. dose funcional).
- [ ] **Solúveis** — Contato inicial com indústrias para envase e mistura inox.

### SEMANA 4 (15/04 → 21/04) — "FRIENDS & FAMILY + GO LIVE"

- [ ] **Protótipos** — Receber protótipos físicos (sucos embalados + cápsulas encaixotadas).
- [ ] **Teste F&F** — Distribuir para círculo de confiança: feedback sobre produto, embalagem e branding.
- [ ] **Site** — Go-live do e-commerce (se checkout OK).
- [ ] **B2B** — Montar "Lista de 100 Prospects" prioritários no RJ (academias, empórios, cafeterias, coworkings, aeroportos, shopping).
- [ ] **B2B** — Iniciar prospecção ativa (mínimo 5 contatos/dia).
- [ ] **Logística** — Testar rotina de despacho de pedidos (embalagem de envio, prazo).
- [ ] **Shots** — Decidir se os 4 shots lançam juntos ou em fases.

---

## 🟠 CHECKLIST MÊS 2 (SEMANAS 5-8) — AJUSTES E PRÉ-LANÇAMENTO DA FASE 2 (SHOTS + CAFÉ)

> [!IMPORTANT]
> Essas tarefas rodam em paralelo com as vendas da Fase 1 e refinamento dos testes do círculo F&F.

- [ ] **Semana 5:** Compilar feedback dos testes F&F da Fase 1 — ajustar dose, rótulo, pitch se necessário.
- [ ] **Semana 5:** Agendar produção piloto dos Shots (Vida Líquida) e Café Moído (MushSpresso).
- [ ] **Semana 6:** Disparar produção do **Lote 1 Oficial** (Caixas de 10 Cápsulas + 3 SKUs de Suco em volume).
- [ ] **Semana 6:** Criar e finalizar embalagem dos 4 Shots e do Café Moído (Definir mini-garrafa PET 60ml).
- [ ] **Semana 7:** Contratar/definir **Vendedor B2B** e **Braço Direito** + formulação "Lista 100 Prospects" RJ.
- [ ] **Semana 7:** Validar viabilidade de ingredientes funcionais (Copaíba e Boswellia) e revisar doses com nutricionista.
- [ ] **Semana 8:** Treinamento de vendas (Pitch agressivo da Zhy), alinhamento logístico para suprir os pontos.
- [ ] **Semana 8:** Validar viabilidade técnica final (vida de prateleira) e empacotamento.

---

## 🔵 CHECKLIST MÊS 3 (SEMANAS 9-12) — VENDAS E PRIMEIRA TRAÇÃO

*Colocar o bloco na rua, buscar os primeiros 10 pontos B2B e ligar os motores D2C.*

- [ ] **Semana 9:** Abertura oficial do Site, início agressivo das visitas B2B corpo-a-corpo (Mín. 5/dia).
- [ ] **Semana 9:** Lançar 4 Shots e Café Moído no site + pontos B2B existentes.
- [ ] **Semana 10:** Meta: **5 pontos B2B fechados**. Medir giro nas prateleiras e testar logística na vera.
- [ ] **Semana 11:** Otimizar SAC e rotinas de envio. Lançar campanhas locais de tráfego pago D2C focadas em conversão.
- [ ] **Semana 12:** Meta: **10 pontos B2B ativos**. Identificar quais SKUs (sabores) já precisam de reposição / giraram mais.
- [ ] **Semana 12:** Fechar parceiro industrial de Solúveis (envase + mixer inox, orçamento ~R$120.000).

---

## 🟣 CHECKLIST MÊS 4 (SEMANAS 13-16) — ESCALA E CHECK-OUT FASE 3 (SOLÚVEIS)

*Atingir velocidade de cruzeiro, forçar recorrência no B2B e fechar as 4.000 unidades.*

- [ ] **Semana 13:** Disparar produção do Lote 2 (escala pesada). Cortar SKUs ineficientes com sangue frio.
- [ ] **Semana 14:** Meta: **15 pontos B2B ativos**. Focar incansavelmente na recompra (*rebuy*) baseando-se no que girou.
- [ ] **Semana 15:** Apertar CAC D2C e escalar campanhas vencedoras. Criar embalagens definitivas (Golden Latte, Cacau, Matcha).
- [ ] **Semana 16:** Meta: **20 pontos B2B ativos**. Bater marcador de **4.000 unidades** (3k sucos + 1k caixas cx/10).
- [ ] **Semana 16:** Iniciar testes P&D c/ novo parceiro (textura, dissolução) e validar formulações c/ Conselho Científico.

---

## 📋 DELEGAÇÃO — QUEM FAZ O QUÊ

| Pessoa / Entidade | Responsabilidades Imediatas |
|---|---|
| **Gianluca** | Decisão PET (sucos + shots), compra maquinário, estratégia geral, buscar parceiro solúveis |
| **Guilherme** | Fechar parceiro HPP no RJ (até 28/03!) |
| **Caco / Carol** | Arte embalagem cápsulas (01/04), arte embalagem sucos (03/04), arte 4 shots, fotos, copy |
| **Adriel (MushSpresso)** | Produção 9k cápsulas, lote café moído piloto |
| **Alvaro (Vida Líquida)** | Produção sucos + 4 shots, setup rotuladora + freezer |
| **Rebu** | Marca definitiva + 15 embalagens (4 meses — provisório até lá) |
| **Priscila** | Liberações MAPA (sucos, shots, alimentos inovadores, Copaíba) |
| **Adriana** | Dossiês ANVISA suplementos (roda paralelo) |
| **Web/Dev** | Finalizar e-commerce com checkout funcional |
| **[A CONTRATAR]** | Vendedor B2B + Braço Direito gestão |
| **[A CONSTITUIR]** | **Conselho Científico Zhy** — médico funcional, nutricionista sênior, especialista em cogumelos/fitoterapia. Validação formulações pré-MAPA. **Urgente.** |

---

## 📊 ROTINA DIÁRIA DE GESTÃO

| Quando | O que fazer |
|--------|-------------|
| **Todo dia** | ☐ Checar este plano e marcar tarefas concluídas |
| **Todo dia** | ☐ Mínimo 5 contatos B2B (WhatsApp, visita, ligação) |
| **Todo dia** | ☐ Acompanhar pedidos do site e despachar |
| **Segunda** | ☐ Reunião tática rápida: quais SKUs giraram? Quem pediu reposição? |
| **Quarta** | ☐ Check-in com parceiros (Alvaro, Adriel, Guilherme, Marketing) |
| **Sexta** | ☐ Revisão financeira: o que quebrou? O que faltou? Ajuste de rota para a semana seguinte |

---

## 🗺️ TIMELINE VISUAL

```text
MAR             ABR                                     MAI         JUN         JUL         AGO
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
25      28      03      10      15      20              01          15          01          01
│       │       │       │       │       │               │           │           │           │
│       │       │       │       │       │               │           │           │           │
│       ├─ HPP  │       │       │       │               │           │           │           │
│       │  Cáps.│       │       │       │               │           │           │           │
│       │       ├─ PET  │       │       │               │           │           │           │
│       │       │  Orça │       │       │               │           │           │           │
│       │       │       ├─ Maq. │       │               │           │           │           │
│       │       │       │  Rótu.│       │               │           │           │           │
│       │       │       │       ├─ F&F  │               │           │           │           │
│       │       │       │       │  Test │               │           │           │           │
│       │       │       │       │       ├─ Site         │           │           │           │
│       │       │       │       │       │  Live         │           │           │           │
│       │       │       │       │       │               ├─ 🚀 FASE 1│           │           │
│       │       │       │       │       │               │  Sucos    │           │           │
│       │       │       │       │       │               │  Cápsulas │           │           │
│       │       │       │       │       │               │           ├─ 🚀 FASE 2│           │
│       │       │       │       │       │               │           │  Shots    │           │
│       │       │       │       │       │               │           │  Café     │           │
│       │       │       │       │       │               │           │           ├─ Meta 10  │
│       │       │       │       │       │               │           │           │  PDVs     │
└── HOJE│       │       │       │       │               │           │           │           ├─ 🚀 FASE 3
    25/03       │       │       │       │               │           │           │           │  Solúveis
```

### 📍 Marcos-Chave

| Data | Marco |
|------|-------|
| **28/03** | HPP fechado + arte cápsulas aprovada |
| **03/04** | PET (sucos + shots) definido e orçado |
| **10/04** | Maquinário (rotuladora + freezers) comprado · Rótulos sucos arte final |
| **15/04** | F&F Test: protótipos em mãos |
| **20/04** | Site com checkout live |
| **01/05** | 🚀 **FASE 1 — Lançamento** (Sucos + Cápsulas) |
| **15/06** | 🚀 **FASE 2 — Lançamento** (4 Shots + Café Moído) |
| **01/07** | Meta: 10 PDVs B2B ativos · 4.000 unidades vendidas |
| **01/08** | 🚀 **FASE 3 — Lançamento** (Linha Solúveis) · 20 PDVs B2B |

---

## 💊 PORTFÓLIO ATUAL — STATUS DE FORMULAÇÕES

| Linha | Produto | Formulação | Status Comercial |
|---|---|---|---|
| ☕ Café | Cápsulas Nespresso (cx 10 un) | ✅ Pronta | 🟡 Próximo ao lançamento |
| ☕ Café | Café Moído Premium (250 gr) | ✅ Pronta | 🟡 Embalagem a resolver |
| 🧃 Cold Press | 6 Sucos (mix sabores) | ✅ Pronta | 🟡 Próximo ao lançamento |
| 💉 Shots | 🛡️ BlindC (Imunidade) — R$9,90 | ✅ Validar | 🔵 Armar conselho científico |
| 💉 Shots | 🔥 Inflamação (Anti-inflam.) — R$9,90 | ✅ Validar | 🔵 Armar conselho científico |
| 💉 Shots | 🧠 Foco (Cognição) — R$9,90 | ✅ Validar | 🔵 Armar conselho científico |
| 💉 Shots | ⚡ Rendimento (Performance) — R$9,90 | ✅ Validar | 🔵 Armar conselho científico |
| 🍵 Solúveis | Zhy Golden Latte — R$69,90 | ✅ Validar | 🔵 Parceiro industrial a buscar |
| 🍵 Solúveis | Zhy Cacau — R$74,90 | ✅ Validar | 🔵 Parceiro industrial a buscar |
| 🍵 Solúveis | Zhy Matcha — R$89,90 | ✅ Validar | 🔵 Parceiro industrial a buscar |
| 🥫 Enlatados | Ice Coffee, Ice Matcha, RTDs... | ⚪ Criar Formulações | ⚪ Fase 4 |
| 💊 Suplementos | Cáps. cogumelos individuais | ⚪ Formular Proteína | ⚪ Fase 5 (ANVISA) |

---

> [!TIP]
> **Como usar este plano:** Abra diariamente. Marque `[x]` nas tarefas concluídas. Nas reuniões de segunda e sexta, revise o progresso e ajuste rotas. Este plano é o seu **comando central** até o lançamento.
